                ...
                query: {
                    attestation: {
                        attestations: jest.fn(hash => {
                            const innerTupleType:Constructor<Codec> = Tuple.with([Text, Text, Text, Bool])
                            const tuple = new Tuple([innerTupleType], [['aClaim', 'anOwner', 'aSignature', false]]);
            ...
            api: {
                query: {
                    attestation: {
                        attestations: jest.fn(hash => {
                            const innerTupleType:Constructor<Codec> = Tuple.with([Text, Text, Text, Bool])
                            return Promise.resolve(new Tuple([innerTupleType], [['aClaim', 'anOwner', 'aSignature', true]]))
                        }),
                    },
                },
            },
