...
describe('CType', () => {
  it('verify model transformations', () => {
    ...
    expect(CType.verifySchemaWithErrors(badClaim, CTypeWrapperModel, [''])).toBeFalsy()
    expect(() => { (new CType(goodClaim)).verifyClaimStructure(goodClaim) }).toThrow(new Error('CType does not correspond to schema'))
    expect(() => { CType.verifyClaimStructure(badClaim, ctypeInput) }).toThrow(new Error('CType does not correspond to schema'))
    expect(() => { CType.fromInputModel(ctypeModel) }).toThrow(new Error('CType input does not correspond to input model schema'))
  })
})
