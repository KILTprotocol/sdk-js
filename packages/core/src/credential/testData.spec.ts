...
import { IAttestation, ICType, ICredential } from '@kiltprotocol/types'
...
export const iCredential: ICredential = {
  claim: {
    contents: {
      birthday: '1991-01-01',
      name: 'Kurt',
...
iCredential.rootHash = calculateRootHash(iCredential)

export const attestation: IAttestation = {
  claimHash: iCredential.rootHash,
  cTypeHash: iCredential.claim.cTypeHash,
  delegationId: iCredential.delegationId,
...
export const credential: KiltCredentialV1 = {
  ...fromInput({
    claims: iCredential.claim.contents,
    claimHash: iCredential.rootHash,
    subject: iCredential.claim.owner,
    delegationId: iCredential.delegationId ?? undefined,
    cType: cType.$id,
  ...
  proof: {
    ...
    commitments: iCredential.claimHashes.map((i) => base58Encode(hexToU8a(i))),
    // salt/nonces must be sorted by statement digest (keys) and base58 encoded
    salt: Object.entries(iCredential.claimNonceMap)
