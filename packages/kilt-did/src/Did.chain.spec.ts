...
describe('write and deactivate', () => {
  ...
  it('writes a new did record to chain', async () => {
    ...
    await expect(
      create(signed).then(async (tx) => {
        ...
        return kilt.BlockchainUtils.submitTxWithReSign(tx, alice, {
  ...
  it('deactivates did from previous step', async () => {
    ...
    const didDeactivate = new (TYPE_REGISTRY.getOrThrow<IDidDeletionOperation>(
...
it('creates and updates did', async () => {
  const id = kilt.Identity.buildFromMnemonic('')
...
afterAll(async () => await kilt.disconnect())
